...
public class MemberArraysList {
	...
	public static void main(String[] args) {
		...
		mlist.add(new Member(202110111,"미나"));
		...
		mlist.add(1, new Member(1990111, "나연"));	//이모모,나연,사나,미나,쯔위
		mlist.remove(3);							//이모모,나연,사나,쯔위
		System.out.println("-----------------------------------");
		for(int i=0;i<mlist.size();i++)
			System.out.println("index= " + i + "element=" + mlist.get(i));
		
		mlist.remove(momo);		//객체의 참조값을 전달하여 삭제합니다. 인자타입이 Object일때 입니다.
		System.out.println("-----------------------------------");	
		for(Member temp : mlist)		
			//변수 i를 사용안할떄, 빠른 for문 또는 빠른(향상된) for문 - for(요소타입선언 변수명 : 리스트 또는 배열변수명)
