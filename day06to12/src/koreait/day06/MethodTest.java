...
public class MethodTest {
	...
	public static void main(String[] args) {
		System.out.println();	
		// System은 클래스, out은 (객체형)필드, println은 out의 메소드
		
		//메소드 실행 - 메소드 이름 호출합니다.
		testMethod1();	//main 메소드의 특징 : static, 실행할수 있는 메소드는 static만 가능합니다.
...
//		testMethod2(123, 123);	//오류 -> 두번째 인자의 형식이 틀림.
		...
		testMethod3();		//반환값(리턴값)이 잇는 메소드는 변수를 사용하여 저장 또는 출력을 합니다.
		...
		System.out.println("\n요일 출력");
		System.out.println(dayOfweek(4));		//반환값은 문자열의 참조값으로 내용(문자열)을 출력.
		System.out.println(dayOfweek(3));
		
	} // man end		
	//1. 인자(args), 반환값이 없는 메소드입니다.
	...
	static	void testMethod1() {		//메소드 정의 연습1	//메소드와 함수는 성격이 유사합니다.
 	...
 	//인자는 개수는 제한이 없습니다.	변수 선언과 같은 방식으로 ,로 구분합니다. 인자는 메소드 실행에 필요한 데이터입니다.
...
// 	3.인자 없고, 반환값이 있는 메소드 입니다.
 	...
 	static int testMethod3() {
		return 0;		//반환값이 있는 형식에서는 꼭 있어야 하는 명령문 -> return
 	...
 	//정수값을 전달 받습니다. 1이면 "월요일", 2이면 "화요일", 3이면 "수요일"... 7이면 "일요일"을 리턴하는 메소드
 	//1~7 이외의 값은 "unknown"	리턴하는 메소드 -> 메소드 이름은 dayOfweek
// 	static void dayOfweek(int a, String name) {
// 		if(a==1) {
