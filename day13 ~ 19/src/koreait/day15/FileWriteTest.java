...
//파일 출력의 기본 : 경로(어디에 있는 파일인지) , 파일명(파일이름)	
	...
	try {	//파일입력 또는 파일출력할때 자바는 꼭 try ~ catch로 처리하도록 합니다.
		...
		pw.close();	//파일작업 종료 : 출력버퍼에서 파일로 전송됩니다. 이게 없으면 파일내용이 작성되지않는다.
		System.out.println("파일 출력이 완료되었습니다.");
		
	}catch (FileNotFoundException e) {	//파일 출력은 파일없으면 자동생성합니다. Exception 안생깁니다.
