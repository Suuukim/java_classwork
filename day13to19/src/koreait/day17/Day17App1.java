...
//		mac4.setVolume(13);		//오류의 원인 : mac4의 참조타입 WebSearch은 sevVolume메소드 없습니다.
		mac4.search("유투브");
		
		
//		필요하다면 casting 으로 메소드를 실행합니다.
		MacBook temp = (MacBook)mac4;		//*주소 전달합니다. : temp와 mac4 참조변수의 저장하는 주소는 같다. 타입만 바꿔준 것.
		temp.setVolume(11);
		...
		//인터페이스의 static 메소드는 Interface로만 사용할 수 있습니다. 그래서 RemoteControl의 changeBattery를 사용하려면 밑에 방식대로 사용합니다.
		RemoteControl.changeBattery();
		
		//인터페이스의 인스턴스 메소드 : RemoteControl을 구현한 객체이면서 참조타입이 RemoteControl 그룹중 하나일때 모두 사용가능합니다.
//		mac4.setMute(false); 	mac1, mac2, mac3는 구현가능함. 참조타입이 RemoteControl과 관계없어서 오류.
