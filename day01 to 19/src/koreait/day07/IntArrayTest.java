...
public class IntArrayTest {		//정수값 배열을 연습합니다.
	...
	public static void main(String[] args) {
		...
		System.out.println("arr1의 배열 길이(크기) : " + arr1.length);
		System.out.println("arr2의 배열 길이(크기) : " + arr2.length);
		...
		for(int i=0;i<arr2.length;i++) {		//i<arr1.length는 배열의 크기가 변경되어도 잘 실행될수 있습니다.
			arr2[i] = r.nextInt(max-min+1)+min;
			System.out.println("arr2[" + i +"]의 데이터 값 : " + arr2[i]); 	//arr[i]는 i번쨰 위치 배열의 값
		}
		
		//배열 특정위치에 데이터를 저장합니다.	arr2배열 5번째 위치에 78을 저장해보세요.
		arr2[4]=78;
		System.out.println("arr2 배열 5번째 데이터의 값 : " + arr2[4]);	//값? 78
		...
		//arr2배열의 값으로 문자열 생성하여 반환합니다.
		
		//발전방향 : arr2 배열의 난수값 만들때 중복된 값은 제외합니다. 
//		그리고 6개를 한쌍으로 5번 만듭니다. -> Lotto
	}
