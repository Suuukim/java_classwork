...
public class CypherTest {
	//연습 예제
	//내용 : 모모가 쯔위에게 메시지를 전달하는데 비밀메시지로 암호화를 합니다.
...
//			temp[i]=tmep[i]+key;	// key는 int로 된 4바이트, temp는 char로 2바이트라서 캐스팅이 필요함.
			temp[i] += key;	//자동 캐스팅해주는 축약연산자 사용. 
		}
		...
		//String.valueOf 메소드는 여러타입 데이터를 문자열로 변환.
