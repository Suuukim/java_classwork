...
import koreait.day18.StudentScore;
...
public class StudentScoreTest {
	...
	public static void main(String[] args) {
		...
		int num = Integer.parseInt(sc.nextLine());		//입력한 문자열을 정수로 바꿔주는 메소드
		
		for(int i=0;i<num;i++) {
		//문제 3) 국어,영어,과확과목의 점수를 난수로 생성하여 
		//		StudentScore 객체 생성과 함께 필드값으로 저장하고 객체는 배열이 참조하는 코딩을 작성하세요.
		...
		for(StudentScore score : stus) {		//for문 실행은 10번(배열의 크기만큼) num<10 일때 오류 발생합니다.
		//문제 4) 배열에 저장된 데이터를 출력하기 위해 필요한 명령을 작성하세요. null은 출력하지 않습니다.
			if(score == null) break;	// 배열값이 10으로 입력되어있기 때문에 학생수가 10이 아니면 null값이 발생할수 있기때문에  null값을 없애기 위해선 이 코딩이 필요하다.
		...
		System.out.println("최우수학생 : " + stus[max_idx].getName() +
							"총점 :  " + stus[max_idx].sum()	
							+ ", 평균 : " + df.format(stus[max_idx].avg()) + ")");
		
//		※ 완성된 StudentScoreTest 클래스를 배열 대신 ArrayList 를 사용하여 구현하세요.( StudentScoreTest2) ※
